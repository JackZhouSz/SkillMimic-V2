...
class CustomDataset(Dataset):
    def __init__(self, motion_dir, history_length=30):
        ...
        self.file_paths = [motion_dir] if os.path.isfile(motion_dir) else [ \
            ...
            for f in filenames 
            if f.endswith('.pt') # 'pickle' in f and 
        ]
        self.data = []
...
# Usage
motion_dir = '/home/runyi/SkillMimic2_yry/skillmimic/data/motions/CVPR_experiment/exp1_planb/'
history_length = 60
...
# TensorBoard Logger
logger = TensorBoardLogger("output/tb_logs", name="comprehensive_model")

# Model training
model = ComprehensiveModel(history_length)
trainer = pl.Trainer(max_epochs=1000, devices=1, logger=logger)
trainer.fit(model, dataloader)

### Command ###
# CUDA_VISIBLE_DEVICES=1 python state_prediction_local_noisy_ry.py
###############
