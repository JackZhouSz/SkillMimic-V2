...
class SkillMimicBallPlayHist(HumanoidWholeBodyWithObject): 
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.ref_hoi_data_size = 9 + self._dof_obs_size*2 + 3 + 6
        ...
        self.hist_encoder = HistoryEncoder(self.history_length).to(self.device)
    ...
    def _update_hist_hoi_obs(self, env_ids=None):
        ...
        current_obs = torch.cat([self._curr_obs[:, :6], # 3 root_pos, 3 root_rot_3d
                                 self._curr_obs[:, 3:6], # 3 redundant root_rot_3d
                                 self._curr_obs[:, 6:6+self._dof_obs_size], # 156 dof_pos
                                 self._rigid_body_pos.reshape(-1, self._dof_obs_size + 3), # 159 body_pos 
                                 self._curr_obs[:,318:318+3], # obj_pos,
                                 torch.zeros_like(self._curr_obs[:,318:318+3]) # zero_obj_rot
                                ], dim=-1)
        # [0, 1, 2, a, b, c, d] -> [1, 2, b, c, d, currect_obs]
        self._hist_obs_batch = torch.cat([self._hist_obs_batch[:, 1:], current_obs.unsqueeze(1)], dim=1)
        #####################################
        return
    ...
    def _compute_observations(self, env_ids=None): # called @ reset & post step
        ...
        # hist_vector = torch.zeros(len(env_ids),3).to(self.device)
    ...
    def _reset_actors(self, env_ids):
        ...
        for ind in range(env_ids.shape[0]):
            ...
            ref_data_obs = torch.cat([ref_data['root_pos'], ref_data['root_rot_3d'], ref_data['root_rot_3d'],
                                      ref_data['dof_pos'], ref_data['body_pos'].view(-1, self._dof_obs_size + 3), 
                                      ref_data['obj_pos'], torch.zeros_like(ref_data['obj_pos'])], dim=-1)
        ...
        if self.cfg['env']['state_init_random_prob'] > 0:
            for ind, env_id in enumerate(env_ids):
                if state_random_flags[ind]:
                    # to ensure the humanoid is above the ground
                    init_root_pos_noise = torch.randn_like(self.init_root_pos[env_id]) * 0.1
                    init_root_pos_noise[2] = torch.abs(init_root_pos_noise[2])
                    self.init_root_pos[env_id] += init_root_pos_noise
                    ...
                    # to ensure the object is above the ground
                    init_obj_pos_noise = torch.randn_like(self.init_obj_pos[env_id]) * 0.1
                    init_obj_pos_noise[2] = torch.abs(init_obj_pos_noise[2])
                    self.init_obj_pos[env_id] += init_obj_pos_noise
                    if self.isTest:
        ...
        if self.cfg['env']['state_switch_prob'] > 0:
            ...
            for ind, env_id in enumerate(env_ids):
                if state_switch_flags[ind] and not state_random_flags[ind]:
                    tmp_motion_id = self._motion_data.sample_switch_motions(1, motion_ids[ind:ind+1])
                    tmp_motion_time, new_motion_time = self._motion_data.resample_time(motion_ids[ind:ind+1], tmp_motion_id)
                    motion_times[ind] = new_motion_time

                    # resample the hoi_data_batch
                    self.hoi_data_batch[env_id], self.init_root_pos[env_id], _,  _, _, _, _, self.init_obj_pos[env_id], _, _, _ = \
                        self._motion_data.get_initial_state(env_ids[ind:ind+1], motion_ids[ind:ind+1], motion_times[ind:ind+1])
                    ...
                    init_obj_pos, self.init_obj_pos_vel[env_id], self.init_obj_rot[env_id], self.init_obj_rot_vel[env_id] \
                        = self._motion_data.get_initial_state(env_ids[ind:ind+1], tmp_motion_id, tmp_motion_time)
                    self.init_root_pos[env_id, :2] = init_root_pos[:,:2]
                    self.init_obj_pos[env_id, :2] = init_obj_pos[:,:2]
                    if self.isTest:
                        print(f"Switched from skill {motion_ids[ind:ind+1]} to {tmp_motion_id} for env {env_id}")
        ####################################################################
    ...
    def _reset_deterministic_ref_state_init(self, env_ids):
        ...
        ######################## Modified by Runyi ########################
        # Random nosie for initial state
        state_random_flags = [np.random.rand() < self.cfg['env']['state_init_random_prob'] for _ in env_ids]
        if self.cfg['env']['state_init_random_prob'] > 0:
            for ind, env_id in enumerate(env_ids):
                if state_random_flags[ind]:
