...
class SkillMimic1BallPlay60Hist(SkillMimic1BallPlay): 
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.ref_hoi_data_size = 1 + 17*3 + 3
    ...
    def get_hist(self, env_ids, ts):
        return self._hist_obs_batch[env_ids].view(env_ids.size(0),-1)
    ...
    def _compute_observations(self, env_ids=None): # called @ reset & post step
        ...
        hist_60 = self.get_hist(env_ids, self.progress_buf[env_ids])
        obs = torch.cat([obs, hist_60], dim=-1)

        # [0, 1, 2, a, b, c, d] -> [1, 2, a, b, c, d, currect_obs]
        current_obs = torch.cat([obj_obs[..., :3], humanoid_obs[..., :1], humanoid_obs[..., 1:157].view(-1,52,3)[:,self._key_body_ids-1,:].view(-1,17*3)], dim=-1) # self._key_body_ids-1, remove root pos
    ...
    def after_reset_actors(self, env_ids):
        ...
        for ind in range(env_ids.shape[0]):
            ...
            ref_data_obs = torch.cat([obj_obs, humanoid_obs[..., :1], humanoid_obs[..., 1:157].view(-1,52,3)[:,self._key_body_ids-1,:].view(-1,17*3)], dim=-1)
