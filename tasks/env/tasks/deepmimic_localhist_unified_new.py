...
class DeepMimicBallPlayLocalHistUnifiedRIS(DeepMimicBallPlayLocalHist): 
    ...
    def _init_with_random_noise(self, env_ids, motion_ids, motion_times): 
        ...
        for ind, env_id in enumerate(env_ids):
            if self.state_random_flags[ind]:
                ...
                self.init_root_rot[env_id] += torch.randn_like(self.init_root_rot[env_id]) * noise_weight
...
class DeepMimicBallPlayLocalHistUnifiedRISBuffernode(DeepMimicBallPlayLocalHistUnifiedRIS):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        super().__init__(cfg=cfg,
                    sim_params=sim_params,
                    physics_engine=physics_engine,
...
def compute_humanoid_reward(hoi_ref: Tensor, hoi_obs: Tensor, hoi_obs_hist: Tensor, contact_buf: Tensor, tar_contact_forces: Tensor, 
    ...
    reward = torch.where((skill_label!=0) & (skill_label!=10), rb + ro, rb)
