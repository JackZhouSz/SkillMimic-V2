...
import torch
class SkillMimicBallPlayLocalHistObjhist(SkillMimicBallPlayLocalHist): 
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        super().__init__(cfg=cfg,
                         ...
                         device_id=device_id,
                         headless=headless)
        self.ref_hoi_data_size = 1 + self._dof_obs_size*2 + 3 #ZQH  + 3
    ...
    def get_hist(self, env_ids, ts):
        # return torch.zeros((env_ids.size(0), self.history_length * 3),device=self.device)
        # return self._hist_obs_batch[env_ids,:,-3:].reshape(env_ids.size(0), -1) #ZQH
        return torch.randn((env_ids.size(0), self.history_length * 3), device=self.device)

    def get_hist_size(self): 
        return -3 + self.history_length * 3 #ZQH
    ...
    def _compute_observations(self, env_ids=None): # called @ reset & post step
        ...
        current_obs = torch.cat([humanoid_obs[..., :157],  self._dof_pos[env_ids], obj_obs[..., :3]], dim=-1) # (envs, 316+3) #ZQH , obj_obs[..., 7:10]
    ...
    def after_reset_actors(self, env_ids):
        ...
        for ind in range(env_ids.shape[0]):
            ...
            obj_obs = get_obj_obs(ref_data['root_pos'], ref_data['root_rot_3d'], ref_data['obj_pos']) #ZQH , ref_data['obj_pos_vel']
...
#     root_rot: torch.Tensor,  # 参考点旋转
#     tar_pos: torch.Tensor,   # 目标点位置
#     tar_vel: Optional[torch.Tensor] = None  # 目标点速度（可选）
# ) -> torch.Tensor:  # 返回值是 Tensor
#     heading_rot = torch_utils.calc_heading_quat_inv(root_rot)
...
#     if tar_vel is not None:
#         local_tar_vel = quat_rotate(heading_rot, tar_vel)
    
#     return torch.cat([local_tar_pos, local_tar_vel], dim=-1)