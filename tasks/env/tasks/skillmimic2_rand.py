...
class SkillMimic2BallPlayRand(SkillMimic2BallPlayUnified):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        # self.similarity_weights = {'root_pos': 1,'root_pos_vel': 1,'root_rot_3d': 1,'root_rot_vel': 1,'dof_pos': 0.25, 'dof_pos_vel': 1,'obj_pos': 2,'obj_pos_vel': 2,'obj_rot': 1,'obj_rot_vel': 1}
    ...
    #     if self.cfg["env"]["enable_buffernode"] and torch.nonzero(self.buffer_steps).numel() != 0:
    #         for buffer_env_id in torch.nonzero(self.buffer_steps).squeeze(-1):
    ...
    def _compute_reward(self): #Z 20250118
        ...
        # # Debug
        # actual_times = self.motion_times_total[invalid_envs] + (self.progress_buf[invalid_envs] - self.buffer_steps_init[invalid_envs]).clamp(min=0) #self._motion_data.envid2sframe
        # zipped_data = zip(invalid_clip_ids.cpu().tolist(), 
        #                   self.progress_buf[invalid_envs].cpu().tolist(), 
        #                   self._motion_data.envid2episode_lengths[invalid_envs].cpu().tolist(),
        #                   actual_times.cpu().tolist(), 
        #                   self._motion_data.motion_lengths[self.motion_ids_total[invalid_envs]].cpu().tolist(), 
        #                   self._motion_data.motion_lengths[self._motion_data.envid2motid[invalid_envs]].cpu().tolist(), 
        #                   )
