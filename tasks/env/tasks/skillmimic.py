...
class SkillMimicBallPlay(HumanoidWholeBodyWithObject): 
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.use_old_reweight = cfg['env']['use_old_reweight']
    ...
    def _load_motion(self, motion_file, switch_motion_file=None):
        ...
        self._motion_data = MotionDataHandler(motion_file, self.device, self._key_body_ids, self.cfg, self.num_envs, 
                                            ...
                                            reweight=self.reweight, reweight_alpha=self.reweight_alpha, use_old_reweight=self.use_old_reweight)
        ...
        if switch_motion_file is not None:
            self._motion_data = MotionDataHandler(switch_motion_file, self.device, self._key_body_ids, self.cfg,
                                                         ...
                                                         self.init_vel, self.play_dataset, use_old_reweight=self.use_old_reweight)
        return
    ...
    def _reweight_motion(self, reset_env_ids):
        ...
        if self.reweight: # and not self.isTest:
            if self.progress_buf_total % self.reweight_intervel == 0 and self.progress_buf_total > 0: # self.reweight_intervel 
    ...
    def _reset_random_ref_state_init(self, env_ids): #Z11
        ...
        # motion_ids = torch.tensor([0]*num_envs, device=self.device, dtype=torch.long)
        # motion_ids[27], motion_ids[32], motion_ids[33], motion_ids[34] = 1, 1, 1, 1
        motion_times = self._motion_data.sample_time(motion_ids)
        # motion_times = torch.tensor([90]*num_envs, device=self.device, dtype=torch.int)
        # motion_times[27], motion_times[32], motion_times[33], motion_times[34] = 2,2,2,2
    ...
    def render(self, sync_frame_time=False, t=0):
        ...
        # for i in [27, 32, 33, 34]:
        #     for j in range(self.num_bodies):
        #         self.gym.set_rigid_body_color(self.envs[i], 0, j, gymapi.MESH_VISUAL, gymapi.Vec3(0., 0., 1.)) 
